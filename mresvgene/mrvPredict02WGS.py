...
import timeit
import operator
import vregMRmodel04WGS as VRmodel
...
def process_blockNN(d):
    ...
    for i in range(len(s)):
        ...
        if (np.abs(int(len(test)/3.)-len(p))<=6) and ( (len(p)>65) and (len(p) < 130)):
            try:
                blastp_cmdline = Blastp(db="./vsCdb/vsCdb", outfmt=6)
                stdout, stderr = blastp_cmdline(stdin=str(p))
            except:
                print "Error in Blast execution:" ,stderr
            ...
            print "stderr=", stderr
            if stdout!="": 
                bl_score = float(stdout.split("\t")[10])
            else: 
                bl_score = 100.
            ...
            for strand in [1,-1]:
                ...
                for record in SeqIO.parse(fbar, "fasta"):
                    ...
                    if self.check_contigs: 
                        if ( record.description.split()[3] not in self.contigs):
                            continue
                    print "record.description=", record.description
                    print "cnts=",record.description.split()[3]
                    print "qbar=", qbar
                    if self.check_contigs: 
                        qbar.remove(record.description.split()[3])
                    if strand == 1:
                        seq=record.seq
                    else:
