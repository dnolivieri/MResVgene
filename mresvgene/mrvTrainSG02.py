...
"""
   dnolivieri:  updated ...7 dec 2015
      ...
      update 6ene2016: 
        - try putting the stochastic gradient 
        - must put some pipeline for feature reduction.
...
class TrainClassifier:
    ...
    def do_training(self):
        ...
        rf = RandomForestClassifier(n_estimators=self.Nestimators, criterion='entropy', oob_score=True)
        #rf = ExtraTreesClassifier(n_estimators=self.Nestimators, bootstrap=True, oob_score=True)

        #rf = GradientBoostingClassifier(n_estimators=self.Nestimators)

        """
        #rf = SGDClassifier(loss="modified_huber", penalty="l2")
        rf = SGDClassifier(loss="log", penalty="l2", n_iter=50)


        rf = SVC(probability=True, kernel='rbf')

        ## here is classifier for SVM
        ...
        # here is the default for stochastic gradient
    ...
    def get_cross_val_score(self, rf):
        #rf = ExtraTreesClassifier(n_estimators=self.Nestimators, bootstrap=True, oob_score=True)
        #rf = RandomForestClassifier(n_estimators=self.Nestimators, oob_score=True)
